...
// Input/Output (I/O) function examples

// printf, scanf, (standard input-output)
#include <stdio.h>

// getch(), kbhit(), (console I/O functions)
#include <conio.h>
...
{ 
	...
	printf("\npress any key > ");
	ch = getch(); // getch works better for char input
	...
	while(1) { // make an infinite loop
		if( kbhit() ) { // check if the keyboard has been hit
			ch = getch(); // if a key is hit then read it
			printf("\nch = %c",ch);
